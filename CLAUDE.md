...
- **Database** — EF Core with SQLite for dev, PostgreSQL for prod. Schema changes go through **EF Core migrations**: add the migration to *both* `SocialAgent.Data.Migrations.Sqlite` and `SocialAgent.Data.Migrations.Npgsql`, or the other dialect silently falls behind. `DatabaseMigrationService` applies pending migrations at startup, and adopts a pre-1.5.0 database (schema present, no `__EFMigrationsHistory`) by recording as applied each migration listed in `DatabaseMigrationService.LegacyMigrations` whose table already exists; `MigrateAsync` then creates what is missing. Pre-1.5.0 databases are not uniform (1.3.x lacks `ProviderTokens`), so never collapse those migrations into one baseline. Migrations added from 1.5.0 on need no entry there. Note: `IHistoryRepository.ExistsAsync()` reports **true on Npgsql even when the history table is absent**, so that detection queries the catalogue directly — verified against a real PostgreSQL 17 server, not just SQLite.
- **Polling** — posts are re-fetched across a trailing `SocialAgent:EngagementRefreshDays` window (default 7) on every poll so their engagement counts stay current; fetching only since the last poll froze each post's counts at its first sighting. Notifications fetch only since the last poll. Providers must therefore tolerate a multi-day `since` cheaply — mind per-post API calls such as Threads insights.
